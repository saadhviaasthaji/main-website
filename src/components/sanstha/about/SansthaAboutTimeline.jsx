    ...
    <section className="py-32 bg-white relative overflow-hidden" ref={containerRef}>
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-12 xl:px-16 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-24">
          <Reveal>
            <span className="text-xs uppercase tracking-widest text-black font-bold mb-4 block">Our Journey</span>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif text-black mb-6 tracking-tight">Major Achievements</h2>
          ...
          <div className="space-y-16 md:space-y-32">
            {timelineEvents.map((event, idx) => (
              <div key={idx} className={`relative flex flex-col md:flex-row gap-8 items-start md:items-center ${idx % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                {/* Dot */}
                <div className="absolute left-4 md:left-1/2 w-4 h-4 rounded-full bg-white border-4 border-black -translate-x-1/2 z-10 hidden md:block shadow-[0_0_0_8px_rgba(0,0,0,0.05)]"></div>
                ...
                <div className={`w-full md:w-1/2 ${idx % 2 === 0 ? 'md:pl-16' : 'md:pr-16 text-left md:text-right'}`}>
                  ...
                  >
                    <span className="text-black font-bold tracking-widest text-sm md:text-base uppercase mb-4 block">{event.year}</span>
                    <h3 className="text-3xl md:text-4xl font-serif text-black mb-6">{event.title}</h3>
                    <p className="text-gray-600 font-sans text-lg md:text-xl leading-relaxed">{event.desc}</p>
                  </motion.div>
                </div>
