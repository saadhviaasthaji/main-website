export const leadershipData = {
  founder: {
    id: "founder",
    name: "Dr. Divesh Ramashankar Mishra",
    designation: "Founder",
    photo: "/assets/pages/sanstha/members/founder.webp", // Placeholder
    intro: "Dr. Divesh Ramashankar Mishra is the Founder of Aastha Sevabhavi Sanstha.",
    background: "Born and brought up in Mumbai, Dr. Mishra has been actively contributing to social service alongside his professional journey. His experience in journalism, reporting and various business fields has given him an opportunity to understand the real needs and challenges faced by society.",
    occupation: "Business - Real Estate",
    education: "M.A - University of Mumbai, LL.B.",
    role: "Founder. As the Founder, Dr. Divesh Ramashankar Mishra plays a key role in shaping the organization’s social-service direction, objectives and long-term vision.",
    journey: "With around 15 years of involvement in social service, he has been associated with initiatives aimed at providing food, clothing and essential support to people in need.",
    achievements: [
      "Honorary Doctorate in Social Service",
      "Kyrgyz Republic International Achiever Award",
      "Various Local Honours & Awards"
    ],
    socialLinks: {
      instagram: "https://www.instagram.com/dr.diveshmishra",
      facebook: "https://www.facebook.com/share/1DjUvWH8Hz/"
    },
    contact: "diveshmishra860@gmail.com"
  },
  president: {
    id: "president",
    name: "Mr. Pratik Divesh Mishra",
    designation: "President",
    photo: "/assets/pages/sanstha/members/president.webp", // Placeholder
    intro: "Mr. Pratik Divesh Mishra is a young Engineer, Entrepreneur, Business Professional and Social Service Leader.",
    background: "Born and brought up in Mumbai, Pratik pursued Computer Science Engineering and began his entrepreneurial journey at a young age. He founded PM Talents Production House at 19.",
    occupation: "Business - PM Talents, Sales Manager",
    education: "BTech in Computer Science",
    role: "President. Involved in the day-to-day management, service activities, digital presence, branding, coordination and organizational development.",
    journey: "Associated with Aastha Sevabhavi Sanstha’s service activities since childhood, carrying the legacy forward by combining traditional service with modern management.",
    achievements: [
      "Founded PM Talents Production House at the age of 19",
      "Entrepreneurial experience in digital branding",
      "Working towards integrating traditional values with modern technology"
    ],
    socialLinks: {
    },
    contact: "saadhviaasthaji@gmail.com"
  },
  members: [
    {
      name: "Adv. Om Maurya",
      designation: "General Secretary",
      photo: "/assets/pages/sanstha/members/gs.webp"
    },
    {
      name: "Mr. Bhavesh Dubey",
      designation: "Treasurer",
      photo: "/assets/pages/sanstha/members/treasurer.webp"
    },
    {
      name: "Mr. Shivam Vishwakarma",
      designation: "Program Coordinator",
      photo: "/assets/pages/sanstha/members/Program Coordinator.webp"
    },
    {
      name: "PM Talents",
      designation: "Media and Communication Coordinator",
      photo: "/assets/pages/sanstha/members/mediahouse.webp"
    }
  ]
};
